  ...
  return (
    <aside className="profile-aside w-64 p-4 bg-base-100 rounded-xl shadow-xl">
      <nav className="flex flex-col gap-2">
        {menuItems.map((item) => (
          <motion.div className="w-full" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} transition={{ duration: 0.2, ease: "easeOut" }} key={item.to}>
            <Link className={`btn w-full justify-start gap-2 ${current === item.to ? "btn-primary text-white" : "btn-ghost hover:bg-base-200"}`} to={item.to}>
              {item.icon}
              {item.name}
            </Link>
          </motion.div>
        ))}
        <motion.div className="w-full" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} transition={{ duration: 0.2, ease: "easeOut" }}>
          <button
            onClick={() => {
              handleLogout();
            }}
