...
export default function SettingsForm() {
  ...
  return (
    <>
      ...
      <Formik initialValues={initialvalues} onSubmit={handleSubmit} validationSchema={settingsValidationSchema} enableReinitialize={true}>
        <Form>
          <div className="flex gap-10">
            <Input label="Ім'я" name="first_name" id="first_name" placeholder="Введіть ім'я" />
            <Input label="Прізвище" name="last_name" id="last_name" placeholder="Введіть прізвище" />
          </div>
          <div className="flex gap-10">
            <Input label="Вік" name="age" id="age" placeholder="Введіть вік" />
          </div>
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} transition={{ duration: 0.2, ease: "easeOut" }} className="btn btn-primary mt-10" type="submit">
            Оновити
          </motion.button>
        </Form>
      </Formik>
